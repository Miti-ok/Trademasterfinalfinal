  ...
  const runAnalysis = useCallback(
    async (payload) => {
      ...
      try {
        ...
        context.setAnalysisPayload(response.data)
    ...
    async (analysisId, materials) => {
      ...
      try {
        ...
        context.updateResults({
          analysisPatch: {
            hs_code: response.data.hs_code ?? context.analysis?.hs_code,
            manufacturing_country:
              response.data.manufacturing_country ?? context.analysis?.manufacturing_country,
            destination_country:
              response.data.destination_country ?? context.analysis?.destination_country,
            declared_value: response.data.declared_value ?? context.analysis?.declared_value
          },
