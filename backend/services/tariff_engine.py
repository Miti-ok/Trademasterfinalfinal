...
class TariffEngine:
    ...
    def calculate_tariff(
        ...
        # -------------------------------------------------
        # Fallback if HS not found (Hackathon safe mode)
        # -------------------------------------------------
        if not tariff_data:
            print(f"⚠️ No tariff found for {normalized_hs}. Applying default duty.")
            tariff_data = {
                "base_duty": 10,
                "additional_duty": 0
            }
        ...
        # -------------------------------------------------
        # Explanation Layer (Judge-Friendly)
        # -------------------------------------------------
        explanation = (
